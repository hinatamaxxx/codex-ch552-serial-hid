...
uint8_t Keyboard_rawPress(__data uint8_t usage) {
  __data uint8_t i;
  if (usage >= 0xE0 && usage <= 0xE7) {
    HIDKey[0] |= (1 << (usage - 0xE0));
  } else if (usage != 0 && usage <= 0xDF) {
    for (i = 2; i < 8; i++) {
      if (HIDKey[i] == usage) break;
      if (HIDKey[i] == 0) { HIDKey[i] = usage; break; }
    }
    if (i == 8) return 0;
  } else return 0;
  return USB_EP1_send(1);
}

uint8_t Keyboard_rawRelease(__data uint8_t usage) {
  __data uint8_t i;
  if (usage >= 0xE0 && usage <= 0xE7) {
    HIDKey[0] &= ~(1 << (usage - 0xE0));
  } else if (usage != 0 && usage <= 0xDF) {
    for (i = 2; i < 8; i++) if (HIDKey[i] == usage) HIDKey[i] = 0;
  } else return 0;
  return USB_EP1_send(1);
}
